        ...
        if (takeInput.hasNextInt()) {
            ...
            if (userProvidedInput > 0 && userProvidedInput <= 100) {
                ...
                if (guessNumber.checkUserGuess(userProvidedInput)) {
                    ...
                    if (userProvidedInput > guessNumber.getGeneratedRandomNumber()) {
                        System.out.println("Guessed number is greater than the number");
                    }
                    if (userProvidedInput < guessNumber.getGeneratedRandomNumber()) {
                        System.out.println("Guessed number is smaller than the number");
                    }
...
class GuessNumber {
    ...
    public boolean checkUserGuess(int userGuessedNumber) {
        ...
        return userGuessedNumber == generatedRandomNumber;
    }
}
